...
def train(X, Y, lr = 0.1, iterations = 3):
    ...
    for i in range(iterations):
        ...
        #第二层卷积
        conv_output_second,conv_cache_second = conv_forward(pool_output_first,W[1],B[1],conv_stride[1],conv_padding[1])
        #ReLU激活函数
        relu_output_second = relu_forward(conv_output_second)
        #池化
        pool_output_second,pool_cache_second = max_pool_forward(relu_output_second, pool_size[1], pool_stride[1], pool_padding[1])
        end_time = time.time()
        run_time = end_time - start_time

        #展平
        flatten_out = flatten(pool_output_second)
        ...
        #恢复展平，准备反向传播
        dX = d_flatten(pool_output_second, dX)
        
        #第二层卷积反向传播
        #池化的反向传播
        dZ = max_pool_backward(dX,pool_cache_second)
        #relu函数的反向传播
        dX = relu_backward(dZ,conv_output_second)
        #卷积层的反向传播
        dX,dW,dB = conv_backward(dX,conv_cache_second)
        W[1] = W[1] - lr * dW
        B[1] = B[1] - lr * dB
        ...
        #卷积层的反向传播
        _,dW,dB = conv_backward(dX,conv_cache_first)
        # W[0] = W[0] - lr * dW
        # B[0] = B[0] - lr * dB
        c = cost(Y_hat, Y)
        costs.append(c)
